  ...
  return (
    <div className="relative">
      <div className="absolute top-0 left-1 z-10 flex items-center justify-center p-6 bg-[#138fb5] text-white rounded-t-lg w-[240px] rounded-b-none h-10">
        <h2 className="text-lg font-semibold text-center">アンケート情報一覧</h2>
      </div>
      <div className="pt-10">
        <div className="bg-white rounded-lg border border-gray-200 p-5">
              ...
              <TableHeader>
                <TableRow className="bg-[#75bacf] hover:bg-[#75bacf]">
                  <TableHead className="w-12 text-white">
                    <Checkbox 
                      checked={selectedItems.length === currentSurveys.length && currentSurveys.length > 0}
              ...
              <TableBody>
                {currentSurveys.map((survey) => (
                  <TableRow
                    ...
                    onClick={() => handleSelectItem(survey.id, !selectedItems.includes(survey.id))}
                  >
                    <TableCell>
                      <Checkbox 
                        checked={selectedItems.includes(survey.id)}
                        onCheckedChange={(checked) => handleSelectItem(survey.id, checked as boolean)}
                      />
                    </TableCell>
                    <TableCell className="font-medium text-gray-900">{survey.id}</TableCell>
                    <TableCell className="text-gray-700">{survey.title}</TableCell>
                    <TableCell className="text-center">
                      <Badge variant="outline" className={`${survey.statusColor} font-bold`}>
                        {survey.status}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-gray-700 text-center">{survey.creator}</TableCell>
          ...
          <div className="mt-10">
            ...
            <div className="mt-4 text-center text-sm text-gray-600">
              {startIndex + 1} - {Math.min(endIndex, allSurveys.length)} / {allSurveys.length} 件
            </div>
          </div>
        </div>
      </div>
