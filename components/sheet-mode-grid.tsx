...
export default function SheetModeGrid({
  ...
  const [resizeType, setResizeType] = React.useState<"column" | "row" | null>(
    null,
  );
  ...
  const getCurrentCellParameters = () => {
    ...
    return (
      data[selectedCell.row]?.cells[selectedCell.col]?.parameters ||
      getDefaultCellParameters()
    );
  };
  ...
  const handleFontChange = (fontFamily: string) => {
    ...
    const newData = [...data];
    newData[selectedCell.row].cells[selectedCell.col].parameters.fontFamily =
      fontFamily;
    setData(newData);
    setSaved(false);
  };
  ...
  const handleFontSizeChange = (fontSize: number) => {
    if (!selectedCell) return;
    const newData = [...data];
    newData[selectedCell.row].cells[selectedCell.col].parameters.fontSize =
      fontSize;
    setData(newData);
    setSaved(false);
  };
  ...
  const handleBoldToggle = () => {
    ...
    const newData = [...data];
    const current =
      newData[selectedCell.row].cells[selectedCell.col].parameters.isBold;
    newData[selectedCell.row].cells[selectedCell.col].parameters.isBold =
      !current;
    setData(newData);
    setSaved(false);
  ...
  const handleItalicToggle = () => {
    ...
    const newData = [...data];
    const current =
      newData[selectedCell.row].cells[selectedCell.col].parameters.isItalic;
    newData[selectedCell.row].cells[selectedCell.col].parameters.isItalic =
      !current;
    setData(newData);
    setSaved(false);
  ...
  const handleUnderlineToggle = () => {
    ...
    const newData = [...data];
    const current =
      newData[selectedCell.row].cells[selectedCell.col].parameters.isUnderline;
    newData[selectedCell.row].cells[selectedCell.col].parameters.isUnderline =
      !current;
    setData(newData);
    setSaved(false);
  ...
  const handleTextAlignChange = (align: string) => {
    if (!selectedCell) return;
    const newData = [...data];
    newData[selectedCell.row].cells[selectedCell.col].parameters.textAlign =
      align;
    setData(newData);
    setSaved(false);
  };
  ...
  const handleCellClick = (rowIndex: number, colIndex: number) => {
    ...
    if (onCellSelect && data[rowIndex] && data[rowIndex].cells[colIndex]) {
      onCellSelect(
        data[rowIndex].cells[colIndex].parameters,
        rowIndex,
        colIndex,
      );
    }
  ...
  const handleCellSave = () => {
    if (editingCell) {
      ...
      newData[editingCell.row].cells[editingCell.col].parameters.value =
        editValue;
      setData(newData);
      setSaved(false);
      ...
      if (onCellUpdate) {
        onCellUpdate(
          newData[editingCell.row].cells[editingCell.col].parameters,
          editingCell.row,
          editingCell.col,
        );
      }
  ...
  const getCellStyle = (cell: ExcelCell) => {
    const style: React.CSSProperties = {
      ...
      border:
        cell.parameters.borderStyle === "solid" ? "1px solid #a6a6a6" : "none",
    };
    return style;
  };
    ...
    (e: MouseEvent) => {
      ...
      if (resizeType === "column") {
        ...
        const newWidth = Math.max(
          50,
          columnWidths[columns[resizeIndex].key] + delta,
        );
  ...
  return (
    <>
      ...
      <div className="flex w-full">
        ...
        <div
          style={{
            minWidth: `${(13 + 1) * 150}px`,
            maxWidth: `${(13 + 1) * 150}px`,
            overflow: "hidden",
          }}
        >
          <table className="border-collapse w-full">
            <thead>
              <tr>
                <th style={{ width: 50, minWidth: 50, position: "relative" }}>
                  #
                </th>
              ...
              `}
                  ...
                  >
                    ...
                    <td
                      ...
                      className={
                        selectedRow === rowIndex ? "row-highlight" : ""
                      }
                    >
                      {rowIndex + 1}
                    </td>
                    {(() => {
                      ...
                      `}
                            style={{
                              ...
                              width:
                                columnWidths[
                                  columns[currentVisibleIndex]?.key
                                ] || 150,
                              minWidth: 50,
                              position: "relative",
                            }}
                            ...
                            editingCell?.col === colIndex ? (
                              <input
                                ...
                                style={{
                                  ...
                                  fontFamily: cell.parameters.fontFamily,
                                  fontWeight: cell.parameters.isBold
                                    ? "bold"
                                    : "normal",
                                  fontStyle: cell.parameters.isItalic
                                    ? "italic"
                                    : "normal",
                                  textAlign: cell.parameters.textAlign as
                                    | "left"
                                    | "center"
                                    | "right",
                                }}
        ...
        >
          <table
            ...
            style={{ minWidth: `${Math.max(columns.length - 13, 1) * 150}px` }}
          >
            <thead>
              <tr>
                {columns.slice(13).map((col, index) => (
                  <th
                    ...
                    className={
                      selectedCol === 13 + index ? "col-highlight" : ""
                    }
                  >
                    {col.key}
                    <div
                      className="absolute top-0 right-0 w-1 h-full cursor-col-resize bg-transparent hover:bg-blue-500"
                      onMouseDown={(e) =>
                        handleColumnResizeStart(e, 13 + index)
                      }
                      style={{ zIndex: 20 }}
                    />
                  </th>
                    ...
                    {(() => {
                      ...
                      `}
                            style={{
                              ...
                              width:
                                columnWidths[
                                  columns[13 + currentVisibleIndex]?.key
                                ] || 150,
                              minWidth: 50,
                              position: "relative",
                            }}
                            ...
                            onDoubleClick={() =>
                              handleCellDoubleClick(rowIndex, 13 + colIndex)
                            }
                          >
                            {editingCell?.row === rowIndex &&
                            editingCell?.col === 13 + colIndex ? (
                              <input
                                type="text"
                                value={editValue}
                                onChange={(e) => setEditValue(e.target.value)}
